...
To simplify login for junior students who typically do not own email addresses, a new logic has been introduced for **automatically generating user accounts** upon creation of a `Student` record.

### Login Credentials Logic

-   **Email**: Automatically generated by combining the student’s ID with `@gmail.com`.  
    Example:  
    If `Student ID = 1001`, then → `Email = 1001@gmail.com`
    
-   **Username**: Set as the student's ID (with all non-alphanumeric characters removed).  
    Example:  
    `Student ID = 1001/23` → `Username = 100123`
    
-   **Password**: Set to the original `Student ID`, even if it contains special characters.
    

> ⚠️ **Note**: Ensure that `Student ID` does not contain slashes or special characters that may break username conventions.
